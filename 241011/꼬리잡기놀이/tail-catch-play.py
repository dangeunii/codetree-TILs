...
for turn in range(1, k + 1):
    ...
    if 1 <= m_turn <= n:
        ...
        for i in range(n):
            # 일단 만나는 점이 있는지 찾기
            # team 번호
            if mapp[n - i - 1][m_turn - n - 1] != 4 and mapp[n - i - 1][m_turn - n - 1] != 0:
                # print(n-i-1, turn-n-1)
                team_n = mapp[n - i - 1][m_turn - n - 1]

                # print("team: " ,teams[team])

                # 그리고 그 해당 좌표의 index 찾아서 point 구하기
                if (n - i - 1, m_turn - n - 1) in teams[team_n]:
                    idx = teams[team_n].index(((n - i - 1), (m_turn - n - 1)))
                    # print(idx)
                    point = (idx + 1) ** 2
                    ## 해당 팀의 머리 꼬리 변경
        ...
        for i in range(n):
            ...
            if mapp[3 * n - m_turn][n - i - 1] != 4 and mapp[3 * n - m_turn][n - i - 1] != 0:
                ...
                #print(teams[team_n])
                if (3 * n - m_turn, n - i - 1) in teams[team_n]:
                    idx = teams[team_n].index(((3 * n - m_turn), (n - i - 1)))
                    #print("idx : ", idx)
                    point = (idx + 1) ** 2
                    teams[team_n].reverse()
                break

    elif 3 * n + 1 <= m_turn <= 4 * n:
        for i in range(n):
            if mapp[i][4 * n - m_turn] != 4 and mapp[i][4 * n - m_turn] != 0:
                team_n = mapp[i][4 * n - m_turn]
                if (i, 4 * n - m_turn) in teams[team_n]:
